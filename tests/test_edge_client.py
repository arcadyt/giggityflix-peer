...
class TestEdgeClient:
    ...
    def edge_client(self):
        ...
        with mock.patch("src.services.edge_client.config") as mock_config:
            # Configure client
    ...
    async def test_connect_and_disconnect(self, edge_client):
        ...
        with mock.patch("src.services.edge_client.grpc.aio") as mock_grpc_aio, \
    ...
    async def test_reconnect(self, edge_client):
        ...
        with mock.patch.object(edge_client, "connect", side_effect=[False, True]) as mock_connect, \
             ...
             mock.patch("src.services.edge_client.asyncio.sleep") as mock_sleep:
    ...
    async def test_register(self, edge_client):
        ...
        with mock.patch.object(edge_client, "_pending_requests", {}) as mock_pending_requests, \
             mock.patch("src.services.edge_client.uuid.uuid4", return_value="test-request-id"), \
             mock.patch("src.services.edge_client.asyncio.wait_for", return_value=response):
    ...
    async def test_register_timeout(self, edge_client):
        ...
        with mock.patch.object(edge_client, "_pending_requests", {}), \
             mock.patch("src.services.edge_client.uuid.uuid4", return_value="test-request-id"), \
             mock.patch("src.services.edge_client.asyncio.wait_for", 
    ...
    async def test_handle_screenshot_capture_request(self, edge_client):
        ...
        with mock.patch("src.services.db_service.db_service") as mock_db_service, \
             mock.patch("src.services.screenshot_service.screenshot_service") as mock_screenshot_service:
