...
class TestPeerApp:
    ...
    def peer_app(self):
        ...
        with mock.patch("src.peer_app.config") as mock_config, \
             mock.patch("src.peer_app.db") as mock_db, \
             mock.patch("src.peer_app.EdgeClient") as mock_edge_client_cls, \
             mock.patch("src.peer_app.MediaScanner") as mock_scanner_cls, \
             mock.patch("src.peer_app.stream_service") as mock_stream_service, \
             mock.patch("src.peer_app.api_server") as mock_api_server, \
             mock.patch("src.peer_app.db_service") as mock_db_service, \
...
async def test_auto_generate_peer_id():
    ...
    with mock.patch("src.peer_app.config") as mock_config, \
         ...
         mock.patch("src.peer_app.EdgeClient") as mock_edge_client_cls, \
         mock.patch("src.peer_app.MediaScanner") as mock_scanner_cls, \
         mock.patch("src.peer_app.uuid.uuid4", return_value="generated-uuid"), \
